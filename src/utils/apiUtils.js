 ...
 * In development, this will be relative to use the proxy
 * In production, this will be the absolute URL of your Elastic Beanstalk API
 */
...
export async function safeJsonFetch(url, options = {}) {
    ...
    try {
        ...
        const response = await fetch(fullUrl, {
            ...options,
            // Always include credentials to support cookies if needed
            credentials: 'include',
            headers: {
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
            throw new Error(`Network error: Could not connect to the API server. Check if CORS is enabled on your backend and the API URL is correct (${API_BASE_URL}).`);
        }
...
export async function testApiConnection() {
    ...
    try {
        ...
        // Try a simple GET request to products endpoint
        ...
        if (response.ok) {
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                console.log(`Received ${Array.isArray(data) ? data.length : 'non-array'} response`);
            } else {
